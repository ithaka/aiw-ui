      ...
      map((results: ImageGroup) => {
        if ('id' in results) {
          ...
          this.ig.access.forEach((accessObj) => {
            ...
            if ( (accessObj.entity_identifier === this.user.baseProfileId && accessObj.entity_type === 100) || (this._auth.getUser() && accessObj.entity_identifier === this._auth.getUser().institutionId.toString() && accessObj.entity_type === 200) ){
