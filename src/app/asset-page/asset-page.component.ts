...
export class AssetPage implements OnInit, OnDestroy {
  ...
  private downloadType: string = "image"
    ...
    if (asset && asset['error']) {
      ...
      if (assetIndex == 0) {
        ...
        this.meta.updateTag({
          ...
          content: asset.formattedMetadata['Description'] && asset.formattedMetadata['Description'][0] ? asset.formattedMetadata['Description'][0] : ''
        }, 'property="og:description"')
        this.meta.updateTag({
          property: 'og:url',
          content: this.generatedImgURL
        }, 'property="og:url"')
        this.meta.updateTag({
          property: 'og:image',
          content: asset.thumbnail_url ? 'https:' + asset.thumbnail_url : ''
        }, 'property="og:image"')
    ...
    if (!isFullscreen) {
      ...
      if (this.isFullscreen !== isFullscreen) {
        ...
        if (this.multiviewItems) {
          this.angulartics.eventTrack.next({
            properties: {
              event: 'multiViewItemFullscreen',
              category: 'multiview',
              label: this.assets[0].id
            }
          });
        }
  ...
  trackCollectionLink(collectionName: string): void {
    this.angulartics.eventTrack.next({
      properties: {
        event: 'metadata_collection_link',
        category: 'metadata',
  ...
  /**
   * trackItemDownload
   * Builds and sends captain's log and Google Analytics events for item downloads.
   * @param assetId
   * @returns void
   */
  ...
  private updateMetadataFromLocal(localData: LocalPCAsset): void {
    if (!localData) {
      return
    } // if we don't have metadata for that asset, we won't run any of the update code
  ...
  private multiViewPageViaArrow(): void {
    if (this.isFullscreen) {
      this.angulartics.eventTrack.next({
        properties: {
          event: 'multiViewFullscreenPageViaArrow',
          category: 'multiview',
          label: this.assets[0].id
        }
      })
  ...
  private multiViewPageViaThumbnail(): void {
    if (this.isFullscreen) {
      this.angulartics.eventTrack.next({
        properties: {
          event: 'multiViewFullscreenPageViaThumbnail',
          category: 'multiview',
          label: this.assets[0].id
        }
      })
      ...
      this.angulartics.eventTrack.next({
        properties: {
          event: 'multiViewPageViaThumbnail',
          category: 'multiview',
          label: this.assets[0].id
        }
      })
  ...
  public closeQuizModeTooltip(): void {
    ...
    this.angulartics.eventTrack.next({
      properties: {
        event: 'quizModeTooltipDismissed',
        category: 'promotion',
        label: this.assetIds[0]
      }
    })
  ...
  private handleDownloadClick(downloadType): void {
    ...
    let assetId = this.assetIds[0]

    if (this.userHasAcceptedTerms()) {
      this.trackItemDownload(assetId)
    } else {
